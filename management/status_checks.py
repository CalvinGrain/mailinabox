...
def load_status_checks_config(env):
    """
    Load the status checks configuration from STORAGE_ROOT/settings.yaml.

    The configuration is stored under the 'status_checks' key in settings.yaml:
      status_checks:
        # Disable web domain DNS checks for all domains
        # (SSL certificate checks will still run for email functionality)
        disable_all_web_checks: true

        # Or disable web domain DNS checks for specific domains
        # (SSL certificate checks will still run for email functionality)
        disabled_web_domains:
          - example.com
          - subdomain.example.org

    Note: Disabling web domain checks only skips DNS resolution checks.
    SSL certificate checks are always performed because they are required
    for email services (SMTP, IMAP) to function properly.

    Returns a dictionary with configuration settings.
    """
    # Load the existing settings.yaml file
    settings = load_settings(env)

    # Get the status_checks section (or empty dict if not present)
    status_checks_config = settings.get('status_checks', {})

    if not isinstance(status_checks_config, dict):
        status_checks_config = {}

    # Ensure disabled_web_domains is a list
    disabled_domains = status_checks_config.get('disabled_web_domains', [])
    if not isinstance(disabled_domains, list):
        disabled_domains = []

    # Get disable_all_web_checks setting (default to False)
    disable_all = status_checks_config.get('disable_all_web_checks', False)
    if not isinstance(disable_all, bool):
        disable_all = False

    return {
        'disabled_web_domains': disabled_domains,
        'disable_all_web_checks': disable_all
    }
...
def get_services():
    return [
        ...
        {"name": "Mail-in-a-Box Management Daemon",
         "port": 10222, "public": False, },
        {"name": "SSH Login (ssh)", "port": get_ssh_port(),
         "public": True, },
...
def run_services_checks(env, output, pool):
    ...
    all_running = True
    fatal = False
    ret = pool.starmap(check_service, ((i, service, env)
                       for i, service in enumerate(get_services())), chunksize=1)
    ...
    # Check fail2ban.
    code, ret = shell(
        'check_output', ["fail2ban-client", "status"], capture_stderr=True, trap=True)
    if code != 0:
...
def check_service(i, service, env):
    ...
    def try_connect(ip):
        ...
        import socket
        s = socket.socket(
            socket.AF_INET if ":" not in ip else socket.AF_INET6, socket.SOCK_STREAM)
        s.settimeout(1)
        try:
    ...
    if service["public"]:
        ...
        if try_connect(env["PUBLIC_IP"]):
            ...
            # IPv4 ok but IPv6 failed. Try the PRIVATE_IPV6 address to see if the service is bound to the interface.
                ...
                output.print_error("%s is running (and available over IPv4 and the local IPv6 address), but it is not publicly accessible at %s:%d." % (
                    service['name'], env['PUBLIC_IPV6'], service['port']))
            else:
                output.print_error("%s is running and available over IPv4 but is not accessible over IPv6 at %s port %d." % (
                    service['name'], env['PUBLIC_IPV6'], service['port']))
        ...
        if not running and service["port"] in {80, 443}:
            output.print_line(shell(
                'check_output', ['nginx', '-t'], capture_stderr=True, trap=True)[1].strip())
    ...
    if ufw[0] == "Status: active":
        ...
        for service in get_services():
            if service["public"] and not is_port_allowed(ufw, service["port"]):
                not_allowed_ports += 1
                output.print_error(
                    "Port {} ({}) should be allowed in the firewall, please re-run the setup.".format(service["port"], service["name"]))
...
def check_free_disk_space(rounded_values, env, output):
    ...
    disk_msg = "The disk has %.2f GB space remaining." % (
        bytes_free/1024.0/1024.0/1024.0)
...
def check_backup(rounded_values, env, output):
    ...
    if backup_config.get("target", "off") == "off":
        output.print_warning(
            "Backups are disabled. It is recommended to enable a backup for your box.")
        return
    else:
    ...
    if backups and len(backups) > 0:
        ...
        bk_date = dateutil.parser.parse(
            most_recent).astimezone(dateutil.tz.tzlocal())
...
def run_network_checks(env, output):
    ...
    _code, ret = shell(
        "check_call", ["/bin/nc", "-z", "-w5", "aspmx.l.google.com", "25"], trap=True)
    if ret == 0:
    ...
    rev_ip6 = ".".join(reversed(IPv6Address(
        env['PUBLIC_IPV6']).exploded.split(':')))
...
def evaluate_spamhaus_lookup(lookupaddress, lookuptype, lookupdomain, output, zen):
    ...
    if zen is None:
        output.print_ok(
            f"{lookuptype} address is not blacklisted by zen.spamhaus.org.")
    elif zen == "[timeout]":
...
def run_domain_checks(rounded_time, env, output, pool, domains_to_check=None):
    ...
    domains_to_check = [
        ...
        if not (
            d.split(".", 1)[0] in {"www", "autoconfig",
                                   "autodiscover", "mta-sts"}
            and len(d.split(".", 1)) == 2
...
def run_domain_checks_on_domain(domain, rounded_time, env, dns_domains, dns_zonefiles, mail_domains, web_domains, domains_with_a_records):
    ...
    if domain == env["PRIMARY_HOSTNAME"]:
        check_primary_hostname_dns(
            domain, env, output, dns_domains, dns_zonefiles)
    ...
    if domain in dns_domains:
        check_dns_zone_suggestions(
            domain, env, output, dns_zonefiles, domains_with_a_records)
    ...
    for label in ("www", "autoconfig", "autodiscover"):
        ...
        if subdomain in web_domains or subdomain in mail_domains:
            ...
            subdomain_output = run_domain_checks_on_domain(
                subdomain, rounded_time, env, dns_domains, dns_zonefiles, mail_domains, web_domains, domains_with_a_records)
...
def check_primary_hostname_dns(domain, env, output, dns_domains, dns_zonefiles):
    ...
    for zone in dns_domains:
        if (zone == domain or domain.endswith("." + zone)) and query_dns(zone, "DS", nxdomain=None) is not None:
            has_dnssec = True
            check_dnssec(zone, env, output, dns_zonefiles,
    ...
    if ns_ips == env['PUBLIC_IP'] + '/' + env['PUBLIC_IP']:
        output.print_ok("Nameserver glue records are correct at registrar. [ns1/ns2.{} ↦ {}]".format(
    ...
    if ip == env['PUBLIC_IP'] and not (ipv6 and env['PUBLIC_IPV6'] and ipv6 != normalize_ip(env['PUBLIC_IPV6'])):
        output.print_ok("Domain resolves to box's IP address. [{} ↦ {}]".format(
    ...
    existing_rdns_v6 = query_dns(dns.reversename.from_address(
        env['PUBLIC_IPV6']), "PTR") if env.get("PUBLIC_IPV6") else None
    if existing_rdns_v4 == domain and existing_rdns_v6 in {None, domain}:
        output.print_ok("Reverse DNS is set correctly at ISP. [{} ↦ {}]".format(
    ...
    check_alias_exists("Hostmaster contact address",
                       "hostmaster@" + domain, env, output)


def check_alias_exists(alias_name, alias, env, output):
    mail_aliases = {address: receivers for address,
                    receivers, *_ in get_mail_aliases(env)}
    if alias in mail_aliases:
        if mail_aliases[alias]:
            output.print_ok(
                f"{alias_name} exists as a mail alias. [{alias} ↦ {mail_aliases[alias]}]")
        else:
            output.print_error(
...
def check_dns_zone(domain, env, output, dns_zonefiles):
    ...
    correct_ip = "; ".join(sorted(get_custom_dns_records(
        custom_dns_records, domain, "A"))) or env['PUBLIC_IP']
    ...
    if existing_ns.lower() == correct_ns.lower():
        output.print_ok(
            f"Nameservers are set correctly at registrar. [{correct_ns}]")
    elif ip == correct_ip:
    ...
    if custom_secondary_ns and not probably_external_dns:
        # Explicitly ask the local dns server.
        SOARecord = query_dns(domain, "SOA", at=env['PUBLIC_IP'])
        ...
        for ns in custom_secondary_ns:
            ...
            if not ns_ips or ns_ips in {'[Not Set]', '[timeout]'}:
                output.print_error(
                    f"Secondary nameserver {ns} is not valid (it doesn't resolve to an IP address).")
                continue
            ...
            if ip == correct_ip:
                output.print_ok(
                    f"Secondary nameserver {ns} resolved the domain correctly.")
            elif ip is None:
                output.print_error(
                    f"Secondary nameserver {ns} is not configured to resolve this domain.")
                # No need to check SOA record if not configured as nameserver
                checkSOA = False
            elif ip == '[timeout]':
                output.print_error(
                    f"Secondary nameserver {ns} did not resolve this domain, result: {ip}")
                checkSOA = False
            else:
                output.print_error(
            ...
            if checkSOA:
                ...
                if SOARecord == SOASecondary:
                    output.print_ok(
                        f"Secondary nameserver {ns} has consistent SOA record.")
                elif SOASecondary == '[Not Set]':
                    output.print_error(
                        f"Secondary nameserver {ns} has no SOA record configured.")
                elif SOASecondary == '[timeout]':
                    output.print_warning(
                        f"Secondary nameserver {ns} timed out on checking SOA record.")
                else:
...
def check_dnssec(domain, env, output, dns_zonefiles, is_checking_primary=False):
    ...
    alg_name_map = {'7': 'RSASHA1-NSEC3-SHA1',
                    '8': 'RSASHA256', '13': 'ECDSAP256SHA256'}
    ...
    with open(ds_file, encoding="utf-8") as f:
        for rr_ds in f:
            ...
            dnssec_keys = load_env_vars_from_file(os.path.join(
                env['STORAGE_ROOT'], f'dns/dnssec/{alg_name_map[ds_alg]}.conf'))
    ...
    ds = [tuple(str(rr).split(" "))
          for rr in ds if len(str(rr).split(" ")) == 4]
    ...
    if len(ds) == 0:
        ...
        if matched_ds:
            ...
            if len([r for r in matched_ds if r[1] == '13' and r[2] in {'2', '4'}]) > 0:
                output.print_ok(
            ...
            if is_checking_primary:
                output.print_error(
                    f"""The DNSSEC 'DS' record for {domain} is incorrect. See further details below.""")
                return
    ...
    preferred_ds_order = [(7, 2), (8, 4), (13, 4), (8, 2), (13, 2)
    ...
    for i, ds_suggestion in enumerate(sorted(expected_ds_records.values(), key=preferred_ds_order_func, reverse=True)):
        ...
        output.print_line(
            "Algorithm: {} / {}".format(ds_suggestion['alg'], ds_suggestion['alg_name']))
        output.print_line(
            "Digest Type: {} / {}".format(ds_suggestion['digalg'], ds_suggestion['digalg_name']))
    ...
    if len(ds) > 0:
        ...
        for rr in sorted(ds):
            output.print_line(
                "Key Tag: {}, Algorithm: {}, Digest Type: {}, Digest: {}".format(*rr))
...
def check_mail_domain(domain, env, output):
    ...
    if mxhost is None:
        ...
        if domain == env['PRIMARY_HOSTNAME']:
            output.print_ok(
                f"Domain's email is directed to this domain. [{domain} has no MX record, which is ok]")
        ...
        if valid == postfix_mta_sts_resolver.resolver.STSFetchResult.VALID:
            ...
            if policy[1].get("mx") == [env['PRIMARY_HOSTNAME']] and policy[1].get("mode") == "enforce":
                output.print_ok("MTA-STS policy is present.")
            else:
                output.print_error(
                    f"MTA-STS policy is present but has unexpected settings. [{policy[1]}]")
        else:
    ...
    if dbl is None:
        ...
        output.print_warning("Incorrect spamhaus query: {}. Could not determine whether the domain {} is blacklisted.".format(
...
def check_web_domain(domain, rounded_time, ssl_certificates, env, output, status_checks_config=None):
    # Load config if not provided
    if status_checks_config is None:
        status_checks_config = load_status_checks_config(env)

    # Check if DNS checks should be skipped for this domain
    skip_dns_checks = False

    if status_checks_config.get('disable_all_web_checks', False):
        output.print_warning(
            f"Web domain DNS checks are disabled for all domains (configured in settings.yaml)")
        skip_dns_checks = True
    elif domain in status_checks_config.get('disabled_web_domains', []):
        output.print_warning(
            f"Web domain DNS checks are disabled for {domain} (configured in settings.yaml)")
        skip_dns_checks = True

    # See if the domain's A record resolves to our PUBLIC_IP. This is already checked
    # for PRIMARY_HOSTNAME, for which it is required for mail specifically. For it and
    # other domains, it is required to access its website.
    if not skip_dns_checks and domain != env['PRIMARY_HOSTNAME']:
        ok_values = []
        ...
        output.print_ok("Domain resolves to this box's IP address. [{} ↦ {}]".format(
...
def check_ssl_cert(domain, rounded_time, ssl_certificates, env, output):
    ...
    tls_cert = get_domain_ssl_files(
        domain, ssl_certificates, env, allow_missing_cert=True)
    if tls_cert is None:
    ...
    if cert_status == "OK":
        ...
        output.print_ok(
            "TLS (SSL) certificate is signed & valid. " + cert_status_details)
        ...
        output.print_error(
            "The TLS (SSL) certificate has a problem: " + cert_status)
...
def list_apt_updates(apt_update=True):
    ...
    # it would do.
    simulated_install = shell(
        "check_output", ["/usr/bin/apt-get", "-qq", "-s", "upgrade"])
    pkgs = []
    for line in simulated_install.split('\n'):
        ...
        if m:
            pkgs.append({"package": m.group(1), "version": m.group(
                3), "current_version": m.group(2)})
        else:
            pkgs.append({"package": "[" + line + "]",
...
def check_miab_version(env, output):
    ...
    if config.get("privacy", True):
        ...
        if this_ver == latest_ver:
            output.print_ok(
                f"Mail-in-a-Box is up to date. You are running version {this_ver}.")
    ...
    if os.path.exists(cache_fn):
        ...
        for category, cur_lines in cur_status.items():
            if category not in prev_status:
                ...
                diff = SequenceMatcher(None, stringify(
                    prev_lines), stringify(cur_lines)).get_opcodes()
